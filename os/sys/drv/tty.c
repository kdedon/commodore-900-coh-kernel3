...
#include <sys/tty.h>
#include <errno.h>
/* 4.x termio path (was #ifdef _I386; MI code, adopted for Z8001) */
#include <termio.h>
#include <sys/inode.h>
#include <sys/ascii.h>

/* 4.x clist names for the 3.2 queue primitives */
#define	cltgetq(q)	getq(q)
#define	cltputq(q, c)	putq(q, c)

/*
 * Validated user copies (k71: ttioctl may not write kernel/user-invalid
 * memory).  The i386 kucopyS/ukcopyS live in its mem code; here they are
 * useracc-gated wrappers returning 0 on a bad user address.
 */
static int
kucopyS(k, uv, n)
char *k, *uv;
...
register IO *iop;
{
	/* Cast: func1/func2 are function pointers (4 bytes) and this call has no
	 * prototype, so a bare 0 pushes 2 bytes and ttread0 reads the rest of
	 * each one off whatever follows on the stack -- func2 then came out
	 * nonzero and the queue-full path called it. */
	ttread0(tp, iop, (int (*)())0, (char *)0, (int (*)())0, (char *)0);
}
...
/*
 * The VTIME timer node is the reading PROCESS's, not the TTY's.
 *
 * timeout() links the node it is handed into the kernel's global timq[], and
 * stand() walks that list from the clock interrupt, in whatever context is
 * running at the time.  A loadable driver's TTY (/drv/lrtty, /drv/hrtty) lives
 * in the transient driver window (segment OS, base reprogrammed per call by
 * bio.c drvmap/drest), so a node inside it is addressable only while that
 * driver is the one mapped; the walk reads it under some other mapping and
 * follows whatever is there.  A PROC comes from the kernel arena and is mapped
 * for the life of the process, and each process is inside at most one read at
 * a time, so p_vtime is valid for exactly as long as the timer is armed.
 */
#define VTIMER	(&SELF->p_vtime)
...
/* 4.x termio path (was #ifdef _I386; MI code, adopted for Z8001) */
	/* lbolt is a `long' (coh/var.c) and int is 16 bits here, so this
	 * snapshot must be long too.  As an int it held only the low word:
	 * once the machine had been up past 32767 ticks (327 s at HZ=100),
	 * `lbolt - time0' below picked up the discarded high word and came out
	 * 65536 larger than the elapsed time, so the VTIME test succeeded on
	 * its first evaluation and every timed read returned at once. */
	long time0 = lbolt;
...
/* 4.x termio path (was #ifdef _I386; MI code, adopted for Z8001) */
		...
		if (ISBBYB && vtime) {
			if (got_ch || vmin == 0) {
				timing = 1;
				time0 = lbolt;
				timeout(VTIMER, vtime*VTICKS, wakeup,
				  &tp->t_iq);
			}
		}
...
/* 4.x termio path (was #ifdef _I386; MI code, adopted for Z8001) */
	if (timing)	/* turn off the VTIME timer */
		timeout(VTIMER, 0, (int (*)())0, (char *)0);	/* cancel */
...
register IO *iop;
{
	ttwrite0(tp, iop, (int (*)())0, (char *)0, (int (*)())0, (char *)0);
}
...
/* 4.x termio path (was #ifdef _I386; MI code, adopted for Z8001) */
	...
	case TCXONC:
		/*
		 * Suspend and restart output on request, the out-of-band form
		 * of ^S/^Q.  The argument is passed by value, as TCFLSH's is.
		 * 0 and 1 act; 2 and 3 are the input-side pair, which this
		 * discipline has no queue-side stop for, so they are refused
		 * rather than silently ignored.
		 *
		 * TX_OSTOP marks the stop as one a process asked for, which is
		 * what keeps the flow-control cleanup below off it.
		 */
		switch ((int)vec) {
		case 0:
			s = sphi();
			tp->t_flags |= T_STOP;
			tp->t_xflags |= TX_OSTOP;
			spl(s);
			break;
		case 1:
			s = sphi();
...
/*		++outDrain;	Why? - hws - 91/11/22	*/
	...
	/*
	 * T_STOP is set under three conditions:
	 * - a modem control device is awaiting carrier
	 * - a stopc (usually Ctrl-S) character was received
	 * - a process asked for it with TCXONC, which also sets TX_OSTOP.
	 *
	 * If ioctl just put device into RAWIN mode, make sure device
	 * is not still waiting for startc.  A TX_OSTOP stop is not waiting for
	 * startc: it is released by TCXONC alone, which stays reachable in any
	 * mode, so it is left standing here.  Without that test a RAW line --
	 * where ISIXON is false for the whole life of the line, so this runs on
	 * every ioctl and not just the one that changed the mode -- would lose
	 * the stop on the next unrelated ioctl.
	 */
#if _I386
...
/*
 * Kernel speeds are sgtty codes (0..19, sgtty.h V7 flavor -- what the
 * C900 SCC driver's baud table indexes); the termio ABI carries the
 * canonical termio CBAUD codes (0..15, termio.h standalone values).
 * B2000/B3600/B7200 have no termio name and round down.
 */
static char sg2tiospeed[20] = {
	0, 1, 2, 3, 4, 5, 6, 7,		/* B0..B300: identical */
	8, 9, 10,			/* B600 B1200 B1800 */
	10,				/* B2000 -> termio B1800 */
	11,				/* B2400 */
	11,				/* B3600 -> termio B2400 */
	12,				/* B4800 */
	12,				/* B7200 -> termio B4800 */
	13,				/* B9600 */
	14,				/* B19200 */
	14, 15				/* EXTA EXTB */
};
static char tio2sgspeed[16] = {
	0, 1, 2, 3, 4, 5, 6, 7,		/* B0..B300 */
	8, 9, 10,			/* B600 B1200 B1800 */
	12,				/* termio B2400 */
	14,				/* termio B4800 */
	16,				/* termio B9600 */
	17,				/* termio B19200 (EXTA) */
	19				/* termio B38400 (EXTB) */
};
...
struct termio * trp;
{
	...
	{
		register int sp;
		sp = sgp->sg_ispeed & 0x1F;
		if (sp > 19)
			sp = 0;
		trp->c_cflag |= sg2tiospeed[sp];
	}
...
struct tchars * tcp;
{
	...
	sgp->sg_ispeed = tio2sgspeed[trp->c_cflag & 0x0F];
