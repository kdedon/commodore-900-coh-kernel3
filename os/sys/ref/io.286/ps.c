...
a_sys_parm sp;
a_init_table it0, it1, it2, it15;

char fcn0[0x10], fcn1[0x50], fcn2[0x40], fcn15[0x58];

extern int com_data,i0, i1, i2, i15, fcn_trn0, fcn_trn1, fcn_trn2, fcn_trn15; 
...
init_abios()
{
	...
	codeseg = ptov(0x0600L, 0xffffL); /* Set up an r/w alias to the cs */

					/* Fix the segments in the CDA */
					/* the offsets are still ok */
	tmp1[9] = tmp1[13] = tmp1[17] = tmp1[21] = FP_SEL(codeseg);

	dev1p = tmp[6];
	dev2p = tmp[10];	/* Set this up so calls are easier later */
	 ...
	 */
					...
					/* Fix the segments in the CDA */
	tmp1[11] = tmp1[15] = tmp1[19] = tmp1[23] = sds;

	tmp1[10] = (unsigned)fcn0;	/* Now fix the offsets */	
	tmp1[14] = (unsigned)fcn1;
	tmp1[18] = (unsigned)fcn15;
	tmp1[22] = (unsigned)fcn2;

	fcn1p = tmp[7];
	fcn2p = tmp[11];	/* Set this up so calls are easier later */

	/* Now convert the pointers in the ftts to protected mode pointers */

				      /* This is the location of the Abios */
	fkcopy(cscolon(&fcn_trn0), &tmpp, 4);
	tmpp = (tmpp & 0xffff0000L) >> 12;
	prot = ptovx(tmpp); 
	...
	{			/* Redo the pointer using the saved size */
		tmp = l_com_data + i + 2;
		*tmp = ptov(*tmp, (long)*(unsigned *)(tmp-2));
...
/*	disk_function(START_P); */
	...
	while (rb.ret_code == 2)	/* Wait for time */
	{
		...
		printf("PS: reset - %x\n", rb.ret_code);
		for(i=0L; i < rb.vars.f8.wait_time; i+=4)
			;
 ...
 * psstatus()
 *
 *	Print out appropiate error message
 */
static void
psstatus()
{
	...
	printf("ps%d%c: bno=%U head=%u cyl=%u <",
		ps.ps_drv,
		(bp->b_dev & SDEV) ? 'x' : ps.ps_partn % NPARTN + 'a',
		(bp->b_count/BSIZE) + bp->b_bno + ps.ps_caching 
				- ps.ps_nsec, ps.ps_head, ps.ps_cyl);

	switch (rb.ret_code | 0x00ff) {
		case 0x00aa:
...
void disk_functionb()
{
/*	printf("rb.ret_code =  %x\r", rb.ret_code); */
	while (rb.ret_code == 2)	/* Wait for time */
	{
		long i;
		for(i=0L; i < rb.vars.f8.wait_time; i+=128)
			;
