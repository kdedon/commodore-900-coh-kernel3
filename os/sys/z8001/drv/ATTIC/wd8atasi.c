 ...
 * made table changes for Atasi 42MB hard disk. (nrb)
 */
...
WDP	wdparam = {			/* patch these values as needed: */
	...
	(7 << 4) | (704/256),		/* 7 heads, MSB of cyl */
	704 % 256,			/* # cyl LSB */
	352 / 16,			/* precomp starts at cyl */
	352 / 16,			/* reduced write cur. at cyl */
...
#define	WDCMDBLKPADDR	0x80000L	/* default to 8|0000 based RAM */
#endif
#ifndef	WDBUFPADDR
#define	WDBUFPADDR	0x80400L	/* above cmd blk and ECC buffer */
#endif
...
register WDCMD	*cbp;
{
	...
	if ((d = drive(minor(bp->b_dev))) >= NHDISKS)
		printf("cf: floppy disk #%d: ", d-NHDISKS);
	else
		printf("wd: hard disk #%d", d);
...
dev_t	dev;
{
	printf("wdfformat: format device %x\n", dev);
}
