...
MMU	=	0x00FC			/ Access MMU # 1 (T#2 is unused)
SSTEP	=	0x0203			/ single step logic port address
HRCONF	=	0x0404			/ hi-res video config register: READ = board id
HRIDM	=	3			/ its F1:F0 field, naming the variant
...
cmdblk_:
	...
	jp	(rr2)
	.blkb	[3*512]+cmdblk_-.	/ cmdblk size is 3 512 byte blocks
					/ first for commands, second for ECC,
					/ third is I/O buffer
psa:
	ldar	rr2, start		/ Gets segment number properly
	jp	(rr2)
...
/ Segments 30..35 are the kernel image, system only (machz8001.h NKSEG):
/   text takes as many as it needs from 30 up and data has the next one, so
/   which is which is a property of the LINK, not of this table -- kboot's
...
/ end of the U-area.
	...
	.globl	start			/ exported so ld -e can target it (ROM entry)
...
/ Load system segment lengths.
	...
	call	vidsel			/ Probe video RAM, choose console driver

/ Set up U-area mapping at corebot
/ corebot is the 3.2 MI's 4-byte paddr_t (var.c) holding a physical
/ BYTE address; the Z8010 base field is in 256-byte units, so >>8.

	ldl	rr0, corebot_		/ mapping base (physical bytes)
	srll	rr0, $8			/ to Z8010 base units (256 B); in r1
	ld	r2, $SS
	soutb	MMU+0x0100, rh2		/ Set segment number to U-area
	soutb	MMU+0x0800, rh1		/ Write base field high
	soutb	MMU+0x0800, rl1		/ Write field low

	call	lpclk			/ Initialise clock and LP

	call	hrreport_		/ what vidsel found on the video board
...
vint:
	...
	ldl	ddtregp_, sp		/ For ddt to access registers
...
/ been issued which will turn vectored interrupts on again.
	...
	outb	SSTEP, rl0		/ Single stepper (just before iret)
0:
	iret				/ At last
...
/ NOTE: for speed, ukcopy has a special entry point here.
/
...
pfix_:
	...
	subb	rl0, rl0		/ clear segment # low byte
...
/ Clear segments descriptors with CPU inhibits
...
/ vidsel - probe for a video card and choose the console driver init loads.
/ Called once from `start' after commodore_ (so the MMU is up) and before
/ main_ copies the icode into user space.  It overwrites the icode-relative
/ offset in the argv[2] slot of init's argument vector -- default driv_no =
/ /drv/notty -- with the driver matching whichever board answers.  Hi-res is
/ preferred, then the lo-res text card, else the serial redirect.
/ Interrupts are still off at this point.
/
/ A frame buffer that answers is not yet a console this kernel can draw on.
/ The hi-res card comes in three jumpered variants and 404H names which one is
/ fitted in its F1:F0 field (1 = 640x400x4 colour, 2 = 1024x800 mono,
/ 3 = 1024x1024 mono); /drv/hrtty renders 1 bit per pixel at 128 bytes per
/ raster, which is variant 2 alone.  The variant the driver accepts is the
/ patchable `hrmode' (commodore.c); the raw byte goes to `hrconf' for
/ hrreport() to print, since nothing can print here.  A board that answers
/ anything else falls through to the text card and then to the serial line: a
/ console that refuses beats one that draws garbage on the wrong geometry.
/
/ The read is meaningful only AFTER vprobe, and only when it reports a variant.
/ vprobe answers from the memory layout -- a framebuffer either responds at
/ 0x3E0000 or it does not -- so it is the authority on whether a board is there.
/ 404H then says WHICH board, and F1:F0 = 0 means "no card": a board that
/ answered vprobe cannot also be absent, so a zero here is a read side that is
/ not driven, and the probe stands.  Only a variant hrtty cannot plot on -- 1
/ (640x400x4 colour) or 3 (1024x1024) -- hands the console to the next board.
vidsel:
	ld	r0, $0x3E00		/ hi-res framebuffer 0x3E0000 >> 8
	call	vprobe
	jr	eq, 1f			/ absent: try the text card
	inb	rl0, HRCONF		/ the board's own identity
	subb	rh0, rh0
	ld	hrconf_, r0		/ raw, for hrreport()
	and	r0, $HRIDM
	jr	eq, 2f			/ undriven read side: vprobe found the board
	cp	r0, hrmode_		/ the one variant hrtty can draw on
	jr	ne, 1f			/ another board: leave its screen alone
2:	ld	r0, $driv_hr-icode
	ld	icarg2_, r0		/ patch argv[2] offset -> /drv/hrtty
	ret
1:	ld	r0, $0x3700		/ lo-res text framebuffer 0x370000 >> 8
	call	vprobe
	ret	eq			/ absent: keep the default /drv/notty
	ld	r0, $driv_lr-icode
	ld	icarg2_, r0		/ patch argv[2] offset -> /drv/lrtty
	ret

/ vprobe - r0 = MMU base field (physical base >> 8) of a framebuffer.  Map
/ the transient segment ES onto it and write/read-back 0x55AA then its
/ complement, restoring the original word.  Returns with Z SET when the card
/ is ABSENT: both patterns must survive to call it present, and there is no
/ system RAM in the framebuffer windows, so a match is conclusive.
/ Clobbers r0-r6 and leaves ES remapped (pfix reprograms it per use).
vprobe:
	ld	r1, $ES
	soutb	MMU+0x0100, rh1		/ select segment ES
	soutb	MMU+0x0800, rh0		/ base field high
	soutb	MMU+0x0800, rl0		/ base field low
	sub	r3, r3			/ offset 0 within ES
	ld	r4, ES|0(r3)		/ save the original word
	ld	r5, $0x55AA
	ld	ES|0(r3), r5
	ld	r6, ES|0(r3)
	cp	r6, r5
	jr	ne, 8f			/ mismatch: absent
	ld	r5, $0xAA55
	ld	ES|0(r3), r5
	ld	r6, ES|0(r3)
	cp	r6, r5
	jr	ne, 8f
	ld	ES|0(r3), r4		/ restore
	ld	r0, $1
	test	r0			/ present: clear Z
	ret
8:	ld	ES|0(r3), r4		/ restore
	sub	r0, r0			/ absent: set Z
	ret
...
/ init's argument vector.  Two of its three slots are PATCHED before the icode
/ is copied into user space, and the patch is always an icode-relative OFFSET
/ into one of the strings below -- never a pointer, because this block is about
/ to be copied to a different address.
/
/   argv[1]  the swapper.  init is built NOSWAPPER and ignores it, which is
/            what makes it the free slot: bootpatch() (mdstub.c) puts the boot
/            flags there, "-s" for single user, and the empty string otherwise.
/            Nothing about argc changes, so a slot that is not patched is the
/            empty argument init has always been passed.
/   argv[2]  the console driver.  vidsel (above) patches it from its PROBE, and
/            bootpatch() overrides that when the loader said the operator is on
/            the serial line -- the probe finds the board that is FITTED, which
/            is a different question.
argv:	.word	USEG, file-icode
	.word	USEG
	.globl	icarg1_
icarg1_: .word	swap-icode	/ bootpatch: -> fl_su for single user
	.word	USEG
	.globl	icarg2_
icarg2_: .word	driv_no-icode	/ vidsel/bootpatch: -> driv_hr, driv_lr, driv_no
	.long	0
...
swap:			/ no swapper exec by init
	.byte	0
fl_su:	.ascii	"-s"		/ come up single user (bootinfo.h BF_SINGLE)
	.byte	0
driv_no: .ascii	"/drv/notty"		/ serial console (compiled default)
	.byte	0
driv_hr: .ascii	"/drv/hrtty"		/ hi-res bitmap console (0x3E0000)
	.byte	0
driv_lr: .ascii	"/drv/lrtty"		/ lo-res 6845 text console (0x370000)
	.byte	0
	.even
icend:
icodep_:	.long	icode
...
/ The offsets bootpatch() (mdstub.c) stores into the two slots above.  They are
/ OUTSIDE the icode blob -- past icend -- on purpose: they are the kernel's own
/ constants and must not be copied into the user process, where they would be
/ two words of stray data in the middle of init's argument strings.
	.globl	icflsu_, icdrvno_
icflsu_:	.word	fl_su-icode
icdrvno_:	.word	driv_no-icode
