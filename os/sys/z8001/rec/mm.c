 ...
 */
/* Special hacking by rec 85.VI.06 */
/* Cannot emulate z19 very closely because CBU keyboard driver doesn't */
#define istty kvtty
#define ismmfunc(c)	/* ismmfunc(c) */
#define initkeys()	/* initkeys() */
...
register int c;
{
	...
	case 'I':		/* Reverse Index */
		if (checkspec())  	  /* no action if on line 25 */
   		   return;
		if (mmrow <= 0)		  /* on the top line: scroll down */
			mmscroll(1);
		else {
			--mmrow;
			mmoffset -= 2*NCOL;
...
/*
 * Video ioctl.  The offset the caller supplies is relative to the start
 * of screen memory; pucopy and upcopy address it as a PHYSICAL address,
 * which they map through a segment of their own.
 */
mmioctl(com, vec)
int com;
int vec;
