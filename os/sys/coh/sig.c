...
sigdump()
{
	...
	if ((SELF->p_flags&PFNDMP) != 0)
		return (0);
	/*
	 * IOSYS covers the whole dump, not just the lookup: the name below is a
	 * kernel constant, and the records written after it come from kernel
	 * data (io_base) and from physical memory (IOPHY, set per segment).
	 * The caller is on its way to pexit(), so nothing restores io_seg or
	 * u.u_error afterwards; only a live system call has a return to poison.
	 */
	u.u_io.io_seg  = IOSYS;
	u.u_io.io_flag = 0;
