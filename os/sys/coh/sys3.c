...
char *np;
{
	...
	if ( oflag & O_EXCL ) {
		...
		/* Mark this open inode as exclusive.  The mark is owned by the
		 * file descriptor about to be built: IPEXCL below records it in
		 * f_flag, and iclose() clears IFEXCL when the last reference to
		 * that descriptor goes away -- on close(2), and on process exit
		 * through fdaclose().  Nothing else may clear it, so every path
		 * out of here that does not reach fdinit() must undo it. */
		ip->i_flag |= IFEXCL;
	}
...
int mode;
{
	 ...
	 * set u.u_error on a fault.  A flat base+count/limit pre-check is
	 * neither meaningful against a seg:off pointer nor available here
	 * (udl holds the loadable-driver space limit, not a user-data
	 * limit), so accept and let a real fault report itself.
	 */
	return( 1 );
#else
	register char * end;
