...
seginit()
{
	...
	/*
	 * The swapper is NOT started here.  It is a process, and no process
	 * may be created before the two main() makes: `process' hands out
	 * process ids from cpid, which starts at zero, and pid zero is what
	 * dispatch() and krunch() test for to recognise the idle process --
	 * so whoever calls first becomes the idle process as far as they are
	 * concerned, and init stops being pid 1.  Nor may anything be made
	 * runnable before iprocp exists: the clock is running from main()'s
	 * batflag, and stand() reaches `if (SELF != iprocp) setrun(SELF)'
	 * with SELF still the procq sentinel, dispatching into a queue it
	 * can never return from.  swapinit() (swap.c) is called from
	 * eveinit(), after both.
	 */
}
...
register SEG *sp;
{
	...
	/*
	 * Give back the inode reference ssalloc() took for a shared segment.
	 *
	 * ldetach() is ilock()+idetach(), and the inode is already locked when
	 * a process execs the file it is itself running (pexece holds exlopen's
	 * lock across the loop that frees the old segments), so calling it
	 * there would sleep on a gate this process holds.  Dropping the
	 * reference by hand is what that case needs: the lock is somebody's --
	 * ours -- and whoever holds it holds a reference of their own, so the
	 * count cannot reach zero here and there is nothing for idetach() to
	 * write back or free.  Skipping the decrement instead (which is what
	 * this did) leaked the reference: the inode stayed pinned for the rest
	 * of the boot, its blocks unfreeable after an unlink, and a later
	 * instance of the same program read as ETXTBSY with nothing running.
	 */
	if ((ip=sp->s_ip) != NULL) {
		if (!ilocked(ip))
			ldetach(ip);
			...
			printf("sfree: locked inode dev=%x ino=%d refc=%d\n",
				ip->i_dev, ip->i_ino, ip->i_refc);
	}
	vrelse( sp->s_faddr );
	kfree(sp);
	krunch(0);
...
fsize_t n;
{
	...
	n = bruc(n);		/* segments are click-granular on this MMU:
				 ...
				 * way), so brk-grown segments must round too */
#else
	n += (BSIZE-1);
	n &= ~(BSIZE-1);
#endif
...
register SEG *sp;
{
	...
	pp->p_flags &= ~PFCORE;
	/*
	 * This is the one path that BLOCKS on the swapper rather than merely
	 * benefiting from it, so it pays for the swapper's next scan in full:
	 * without a nudge the segment waits out the rest of the NSRTICK sleep
	 * the swapper is in.  ioreq() and uexit() nudge it the same way.
	 */
	if (stimer.t_last != 0)
		wakeup((char *)&stimer);
#ifndef QWAKEUP
...
#else
		/*
		 * s_faddr is a CLICK BASE on this machine, not a byte pointer
		 * (mdstub.c), and a chunk boundary is a multiple of BSIZE
		 * rather than of a click, so there is no faddr that names the
		 * middle of a transfer.  The disk driver takes its physical
		 * address from b_paddr, set per chunk above; b_faddr keeps
		 * naming the whole region, which is the only thing it can
		 * truthfully say.
		 */
#endif
		p += nb;
...
sproto()
{
	...
	for (n=0; n<NUSEG; n++) {
		...
		/* SRFDATA is what iomapvp() accepts as a raw-I/O destination
		 * (bio.c), so it must name only segments the process may be
		 * written into.  The shared part of a shared library is read-
		 * only text held in common by every client: raw I/O there
		 * would rewrite it for all of them, through the physical
		 * mapping, past the read-only attribute the MMU enforces.
		 *
		 * SFSHRX names every such segment, whichever slot it sits in.
		 * The slot is not enough: an LF_SHR image linked WITHOUT
		 * LF_SEP has its shared text in SISDATA (exec.c), which no
		 * list of excluded indices reaches, and uproto() maps exactly
		 * the SFSHRX segments read-only. */
 ...
 *
 * SFSHRX alone, not SFSHRX|SFTEXT: the test asks "is a segment of this file
 * shared with somebody", and every such segment carries SFSHRX -- it is the
 * flag under which ssalloc() registers s_ip in the first place, and the one
 * uproto() maps read-only.  SFTEXT is set only by the LF_SEP arms of exec, so
 * demanding it let an LF_SHR image linked without LF_SEP (shared text in
 * SISDATA) through with no ETXTBSY at all: iaccess() (fs1.c) would grant write
 * access to a file whose text every running instance is executing from one
 * physical copy, and the overwrite would land under all of them.
 */
sbusy(ip)
register INODE *ip;
{
	 ...
	 */
	for (sp=segmq.s_forw; sp!=&segmq; sp=sp->s_forw) {
		if (sp->s_ip==ip && (sp->s_flags&SFSHRX)!=0) {
			unlock(seglink);
			return (1);
		}
	}
