...
#endif
/*
 * The release string the boot banner and uname(2) report, supplied by the
 * build from hostbuild/VERSION (link-kernel.sh's cc_one passes -DRELEASE for
 * main.c).  No fallback literal: one drifted to "3.5" while hostbuild/VERSION
 * had already moved to 3.5.0, and a stale literal here is silent -- the
 * banner just prints the wrong thing.  A kernel built by hand without
 * -DRELEASE is refused at compile time instead.
 */
#ifndef RELEASE
 ...
 * literal below because this compiler is K&R and adjacent string literals are
 * an ANSI feature -- relying on it here would be relying on an extension in
 * the one file whose output every boot displays.
 */
#ifndef COPYYEAR
#define COPYYEAR "2026"
#endif
...
main()
{
	...
	putchar_init();
	printf("*** OpenCoherent Version %s - %s Mode.  %uKB free memory. ***\n",
	...
	printf(copyright);
	printf("Copyright %s OpenCoherent contributors\n", copyyear);
...
panic("Verification error - call Mark Williams Company at +1-708-291-6700\n");
...
#else
	/*
	 * Uncleared, and it must stay that way on this machine: salloc hands
	 * back corebot, which md.s already mapped as SS -- the live boot
	 * kernel stack and `u' are in it.  eveinit() zeroes the fields that
	 * need it instead.
	 */
	...
	/* The idle process never execs, so nothing else ever writes its
	 * u_comm; ps(1) reads that field to name a kernel process.  eprocp
	 * goes on to exec /etc/init, which names itself. */
	pkname(iprocp, "idle");
	eveinit(sp);
	fsminit();
#endif
