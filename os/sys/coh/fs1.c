 ...
 */
/* $Header: /kernel/kersrc/coh.286/RCS/fs1.c,v 1.1 92/07/17 15:18:05 bin Exp Locker: bin $ */
/* (lgl-
 ...
 *
 * $Log:	fs1.c,v $
 * Revision 1.1  92/07/17  15:18:05  bin
 * Initial revision
 * 
 ...
 * coming from.
 *
 * This is the address space of `u.u_io.io_seg', which the trap path sets to
 * IOUSR on entry to every system call: a path handed to `ftoi' is read out of
 * the CALLER'S data segment unless io_seg says IOSYS.  A kernel string
 * constant is therefore walked at the offset it happens to occupy in kernel
...
/*
 * Map a KERNEL filename to an inode: `ftoi' with the path read out of the
 * kernel's own data segment, whatever the current system call left in
 * `u.u_io.io_seg', which is put back as it was found.  Everything else is
 * `ftoi': the return is `u.u_error', and u_cdiri/u_pdiri/u_cdirn/io_seek are
 * set the same way for each of the three access types.
 *
 * `u.u_error' is deliberately NOT saved here -- the return says whether the
 * lookup found anything, and a caller for which the file is OPTIONAL is
 * containing more than the lookup (the read of it fails the same way), so it
 * brackets the whole operation with its own save and restore.
 */
ftoi_sys(np, t)
char *np;
{
...
register INODE *ip;
{
	...
	case IFPIPE:
		...
		ip->i_pwx = dip->di_pwx;
		canshort(ip->i_pwx);
		pipecoreinit(ip);	/* the fields with no disk image */
		break;
	default:
