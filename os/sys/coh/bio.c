...
*/
		if ((b=iop->io_base) < srp->sr_base)
			continue;
		/*
		 * Byte distance from the segment's virtual base.  A segment
		 * spans consecutive hardware segments, and bits 16..23 of a
		 * virtual address are a hole, so the distance is carried
		 * segment part and offset part separately; the bound check
		 * compares lengths, never summed virtual addresses.
		 */
		d = ((long)(VSEG(b) - VSEG(srp->sr_base)) << 16)
		    + (long)VOFF(b) - (long)VOFF(srp->sr_base);
		if ((fsize_t)d + iop->io_ioc > sp->s_size)
			continue;
		bp->b_paddr = sp->s_paddr + (paddr_t)d;
		return (sp);
	}
	return (NULL);
}
 ...
 * Initialise devices.
 *
 * Called once, from main(), before any driver has been loaded.  Every entry in
 * the static drvl[] sets only d_conp (z8001/con), so d_map is zero throughout
 * and c_load is entered with the driver window untouched -- exactly what
 * drvmap() would leave, since it maps nothing for a d_map of zero.  A loadable
 * driver gets its own c_load from pload(), which has the window installed.
 *
 * The bare call is therefore equivalent here and only here: an entry whose
 * d_map is non-zero would need drvmap()/drest() around it, which is what a
 * driver placed in the static table by hand, or a second devinit() after
 * pload(), would create.
 */
devinit()
{
 ...
 * Poll a device.
 *
 * The driver window is put back before this returns, while the event queues the
 * driver just linked the caller onto outlive the call: a queue head that is a
 * datum of a loadable driver is unreachable from then on.  poll.c records the
 * window in each event buffer for that reason.
 */
dpoll(dev, ev, msec)
register dev_t dev;
int ev;
