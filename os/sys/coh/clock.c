...
static int clocks;
static int lavtick = LAVTICK;	/* Ticks left until the next load sample */

/*
 * Sample the run queue and decay it into the three load averages.
 *
 * The count is of processes running or waiting to run.  procq's n-chain is
 * every process, and PSRUN is the state of one that wants the processor,
 * whether or not it has it; the idle process is always in that state and is
 * not a process anyone is waiting on, so it is not counted.
 *
 * Each average moves a fixed fraction of the way to the count, that fraction
 * being one sample's worth of its time constant (sched.h).  The multiply is
 * long because the fraction is a 16-bit one: the product of a difference that
 * can reach a full scaled count and a fraction near unity does not fit in a
 * word.
 */
static unsigned lavd[NLOADAV] = { LAVD1, LAVD5, LAVD15 };
...
loadav()
{
	...
	for (pp = procq.p_nforw; pp != &procq; pp = pp->p_nforw)
		if (pp->p_state == PSRUN && pp != iprocp)
			nrun++;
	spl(s);
	if (nrun > LAVMAX)
		nrun = LAVMAX;
	for (i = 0; i < NLOADAV; i++) {
		v = ((long)nrun << FSHIFT) - (long)avenrun[i];
		v = (long)avenrun[i] + ((v * (long)lavd[i]) >> 16);
		if (v < 0L)
			v = 0L;
		avenrun[i] = (unsigned)v;
	}
}
...
stand()
{
	...
	do {
		...
		/*
		 * Sample the load average.  Counted here rather than in
		 * clock() so that the walk of the process chain happens at
		 * low priority.
		 */
		if (--lavtick <= 0) {
			lavtick = LAVTICK;
