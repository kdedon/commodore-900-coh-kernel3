...
char *cp;
{
	...
	if (cp != NULL) {
		 ...
		 * the distance from the segment base to `cp' is NOT a plain
		 * subtraction: a break one segment along differs by 0x01000000
		 * where the size differs by 0x10000.  Measure the two parts
		 * separately.
		 *
		 * A data region larger than one hardware segment is already
		 * mappable -- uproto() (z8001/src/commodore.c) walks one software
		 * segment across as many consecutive hardware segments as its
		 * size needs, which is how multi-segment user text works.  What
		 * remains a hard limit is that pointer arithmetic does NOT carry
		 * out of the offset, so no single OBJECT may cross a boundary;
		 * libc's sbrk() answers that by abandoning the tail of a segment
		 * rather than returning a block that straddles one.
		 *
		 * A break BELOW the base is not checked for here: the subtraction
		 * then underflows to an enormous size and seggrow() refuses it
		 * with ENOMEM, which is the right answer by a cheaper route.
		 */
		segsize(sp, ((vaddr_t)(VSEG(cp)-VSEG(sb)) << 16)
			    + VOFF(cp) - VOFF(sb));
#else
