...
/ scantab_ is subr.c's `struct scanline scantab[YMAX]', read here as ONE WORD
/ per entry: high byte sc_off (words of unchanged left edge), low byte
/ sc_nword (words that differ, 0 = nothing to copy).  The stride of 2 below,
/ and the geometry constants above, are a private copy of ../h/display.h and
/ ../h/subr.h -- change either side and this file must change with it.
/ subr.h carries a compile-time check that the struct is still two bytes.
