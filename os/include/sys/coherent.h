...
/*
 * Interruptible sleep that RETURNS 1 on a caught signal instead of unwinding
 * to the system call entry (proc.c).  Spelled with the same descriptive
 * string as v_sleep so the two read alike at the call site; the string is
 * kept only where u_sleep[] exists.
 */
#define v_isleep(a1,a2,a3,a4,a5)	isleep(a1,a2,a3,a4)
