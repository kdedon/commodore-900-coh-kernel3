 ...
 * 3.2: constants the 3.2 MI owns with the same value (BSIZE, HZ, SCHUNK,
 * NPID, MAXU, syscall ranges) are dropped -- <sys/const.h>/<sys/param.h>
 * define them.  Machine parameters where the Z8001 differs from the i286
 * are re-asserted here (#undef first: const.h/param.h are included ahead
 * of this file). */
#undef	MSASIZE
#define	MSASIZE		16		/* Space reserved for sys args */
/* 3.2: ISTSIZE is a tunable (extern int, set in the conf file) -- 256 on
 * the C900, matching the 0.7.3 constant. */
#undef	ISTVIRT
...
#undef	NMDCALL
#define	NMDCALL		3		/* Z8001 MD system calls (sysdtab) */
#undef	NCPCL
#define	NCPCL		(256-sizeof(cmap_t))/* Number of characters in clist */
#define	ADDR(s,o)	(((long)(s)<<24)+(unsigned)(o))
/*
 * The two halves of a virtual address, and addition that carries between them.
 *
 * ADDR puts the segment number in bits 24..30 and the offset in bits 0..15, so
 * bits 16..23 are a HOLE: consecutive addresses do not run consecutively through
 * the encoding.  A virtual address is therefore not a number you can add a byte
 * count to.  Doing it anyway is what made a break past 64K alias the low end of
 * its own segment (sys1.c ubrk), and it is an easy mistake because the arithmetic
 * looks right and is right for every size under 64K.
 *
 * vadd() adds a byte count to a virtual address, carrying whole segments into the
 * segment number.  Segments are MSSIZE clicks; the offset field is what limits a
 * single object, not vadd().
 *
 * Shifts and masks, not division by ctob(MSSIZE): a hardware segment is exactly
 * 65536 bytes, so the quotient is a 16-bit shift and the remainder a mask.  The
 * division reads better and cost 2824 bytes of text -- a 32-bit divide is a
 * runtime call, and there is no room for three of them (the 0x30 code segment
 * has about 70 bytes spare).  If MSSIZE ever stops meaning 64K, this breaks
 * loudly rather than quietly: SEGBYTES is what to fix.
 */
#define	SEGBYTES	0x10000L	/* == ctob(MSSIZE), by construction */
#define	VSEG(a)		((int)(((vaddr_t)(a)) >> 24))
#define	VOFF(a)		((unsigned)(((vaddr_t)(a)) & 0xFFFFL))
#define	vadd(a,n)	ADDR(VSEG(a) + (int)(((vaddr_t)VOFF(a) + (n)) >> 16), \
			     (unsigned)(((vaddr_t)VOFF(a) + (n)) & 0xFFFFL))
#undef	MUERR
#define	MUERR		ADDR(USTACK, 0xFFFE)	/* Location of `errno' */
#define	MSSIZE		64			/* Maximum seg. size clicks */
/* 3.2-contract compatibility for the 0.7.3 MD sources:
 *  - SIPSLIB: the NLD shared-library private slot shares index 3 with
 *    SIPTEXT (a process has one or the other);
 *  - PFSPROTO: "must recalculate segment prototype" -- 0.7.3's 04000
 *    bit is 3.2's PFWAKE, so it moves to the free 0x1000. */
#define	PFSPROTO	0x2000
#define	PFSLIB	0x1000		/* NLD shared-library process (0.7.3 value) */
#define	SIGEPA	12		/* extended-processor (EPU) trap signal (md.s) */
#define	CSH		2			/* Shift saddr_t to hardware */

/*
 * Segments for use in selecting various spaces.
 *
 * IS is the FIRST of the kernel's text segments; ld gives text as many
 * consecutive segments as it needs (main.c newpage) and puts the data in the
 * one after the last, so the DATA segment number is a property of the link,
 * not a constant -- kdaddr() below reads it off a datum rather than naming it,
 * and kboot's launch stub maps whatever the count comes to.  0x30..0x35 are
 * therefore reserved to the kernel image; the mapping segments start above it.
 */
#define	IS	0x30		/* System code segment (first of them) */
 ...
 * address in the PAIR -- vaddr_t is unsigned long -- exactly the shipped
 * kernel's setarena_ codegen.  The 3.2 alloc.h macro block is `#if 0'-ed;
 * machine.h owns these on every port).
 */
#ifdef KERNEL
#define	align(p)	((ALL *) ((vaddr_t) (p) & ~1L))
#define	link(p)		align((p)->a_link)
#define	tstfree(p)	(((int) (p)->a_link&1) == 0)
#define	setfree(p)	((p)->a_link = (char *)((vaddr_t) (p)->a_link & ~1L))
#define	setused(p)	((p)->a_link = (char *)((vaddr_t) (p)->a_link | 1L))
#endif
...
#define	mfixcon(pp)
#define	usave()
/* An offset into KERNEL DATA as a far address.  The data segment number is
 * decided by the link, so take it from a datum that is certainly in it
 * (`end', ld's end-of-bss) rather than naming a constant that a kernel
 * whose text spans two segments would get wrong. */
extern	char	end;
#define	kdaddr(s)	((char *)ADDR(VSEG(&end), (unsigned)(s)))
...
#define	dconv(p)	(p)		/* 3.2: driver map handle conversion */
...
paddr_t	vtop();
vaddr_t	ptov();
extern	saddr_t	uasa;		/* 3.2: currently active u-area segment
				 * (proc.c swap bookkeeping); the Z8001 MD
				 * layer owns its definition */
extern	char	*_bvirt;	/* defined in src/commodore.c; was a duplicate
				 * tentative definition the 4.2.12 front end
				 * rejects (old cc treated it as a common) */
#endif
