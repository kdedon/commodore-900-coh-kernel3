 ...
 * 0.7.3 `b_vaddr' semantics; assigning it to a pointer is a 32-bit pair
 * identity, segment preserved.  FP_SEL/FP_SEG/FP_OFFW access the WORDS of
 * the pair -- big-endian: SEGMENT is word 0, OFFSET is word 1 (the i286
 * macros below assume the little-endian order).  The i286 selector/click
 * machinery below is not used; the MMU translation API (vtop/ptov/...) is
 * supplied by the Z8001 MD layer (machz8001.h declarations).
 */
#define	FP_OFF(f)	(f)
#define	FP_SEL(f)	( ((unsigned short *) &(f))[0] )
